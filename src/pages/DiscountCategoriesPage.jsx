  ...
  return (
    <>
      ...
      <div className="container mx-auto p-4">
        ...
        {editingCategory && (
          <button className="bg-gray-500 text-white px-4 py-2 rounded mb-4 hover:bg-gray-600" onClick={() => setEditingCategory(null)}>
            Batal Edit
          </button>
        )}
            ...
            <ul className="divide-y divide-gray-200">
              {categories.map((category) => (
                <li key={category.discount_category_id} className="py-3 flex justify-between items-center">
                  ...
                  <div>
                    {/* Edit button */}
                    <button className="bg-yellow-500 text-white px-3 py-1 rounded mr-2" onClick={() => handleEditClick(category)}>
                      Edit
                    </button>
                    {/* Delete button */}
                    <button className="bg-red-600 text-white px-3 py-1 rounded" onClick={() => handleDeleteCategory(category.discount_category_id)}>
                      Hapus
                    </button>
                  </div>
                </li>
